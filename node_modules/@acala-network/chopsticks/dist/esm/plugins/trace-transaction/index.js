...
const schema = configSchema.extend({
    ...
    'enable-memory': z.boolean({
        description: 'Enable memory trace'
    }).optional(),
    'disable-stack': z.boolean({
...
export const cli = (y)=>{
    y.command('trace-transaction <tx-hash>', 'EVM+ trace transaction. Only Acala and Karura are supported', (yargs)=>yargs.options(getYargsOptions(schema.shape)).positional('tx-hash', {
        ...
        if (config.vm) {
            ...
            const steps = await traceVM(tracingBlock, extrinsic, config['page-size'], config['disable-stack'], config['enable-memory']);
